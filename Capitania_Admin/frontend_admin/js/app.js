...
function setupFileCapture() {
    ...
    const promoInput = document.getElementById('promocionImagen');
    if (eventoInput && !eventoInput.dataset.bound) {
        eventoInput.dataset.bound = '1';
        eventoInput.addEventListener('change', function() {
            if (this.files && this.files[0]) eventoImagenFile = this.files[0];
        });
    }
    if (promoInput && !promoInput.dataset.bound) {
        promoInput.dataset.bound = '1';
        promoInput.addEventListener('change', function() {
            if (this.files && this.files[0]) promoImagenFile = this.files[0];
        });
    }
}
...
function switchTab(tabId) {
    ...
    const titles = { 'reservas': 'Gestión de Reservas', 'eventos': 'Eventos Especiales', 'promociones': 'Promociones', 'historial': 'Historial del Día' };
...
function filtrarReservas() {
    ...
    if (sucursal) filtered = filtered.filter(r => r.sucursal === sucursal);
...
async function updateEstatus(id, estatus) {
    ...
    try {
        await fetchAuth(`/reservas/${id}/estatus`, { method: 'PUT', body: JSON.stringify({ estatus }) });
        loadReservas();
    } catch (e) { console.error(e); }
}
...
async function submitEvento() {
    ...
    if (file) {
        try {
            ...
            const uploadRes = await fetchAuth('/upload', { method: 'POST', body: formData });
            const uploadData = await uploadRes.json();
            if (uploadData.success) imageUrl = uploadData.imageUrl;
            else throw new Error(uploadData.message || 'Error en upload');
        } catch (uploadError) {
...
async function loadEventos() {
    try {
        ...
        grid.innerHTML = '';
        if (data.length === 0) { grid.innerHTML = `<p style="color: var(--muted);">No hay eventos activos.</p>`; return; }
        data.forEach(ev => {
            const imgSrc = ev.imagen_url ? (ev.imagen_url.startsWith('http') ? ev.imagen_url : 'https://7mzq2kk1-3001.usw3.devtunnels.ms' + (ev.imagen_url.startsWith('/') ? ev.imagen_url : '/' + ev.imagen_url)) : '';
            grid.innerHTML += `<div class="card"><img src="${imgSrc}" alt="${ev.titulo}" class="card-img" onerror="this.onerror=null;this.style.display='none'"><div class="card-body"><h4 class="card-title">${ev.titulo}</h4><p style="font-size: 0.8rem; color: var(--gold); margin-bottom: 0.5rem;">🗓 ${ev.fecha_evento}</p><p class="card-desc">${ev.descripcion}</p><button class="btn-secondary" onclick="abrirEditarEvento(${ev.id})" style="margin-right:0.5rem;">✏️ Editar</button><button class="btn-danger" onclick="deleteEvento(${ev.id})">Eliminar</button></div></div>`;
        });
    } catch (e) { console.error(e); }
}
...
async function loadPromociones() {
    try {
        ...
        grid.innerHTML = '';
        if (data.length === 0) { grid.innerHTML = `<p style="color: var(--muted);">No hay promociones activas.</p>`; return; }
        data.forEach(pr => {
            const imgSrc = pr.imagen_url ? (pr.imagen_url.startsWith('http') ? pr.imagen_url : 'https://7mzq2kk1-3001.usw3.devtunnels.ms' + (pr.imagen_url.startsWith('/') ? pr.imagen_url : '/' + pr.imagen_url)) : '';
            grid.innerHTML += `<div class="card"><img src="${imgSrc}" alt="${pr.nombre}" class="card-img" onerror="this.onerror=null;this.style.display='none'"><div class="card-body"><h4 class="card-title">${pr.nombre} <span style="float:right; color:var(--gold-xl);">${pr.precio_destacado || ''}</span></h4><p class="card-desc" style="margin-top: 1rem;">${pr.descripcion}</p><button class="btn-secondary" onclick="abrirEditarPromo(${pr.id})" style="margin-right:0.5rem;">✏️ Editar</button><button class="btn-danger" onclick="deletePromo(${pr.id})">Eliminar</button></div></div>`;
        });
    } catch (e) { console.error(e); }
}
...
function abrirEditarPromo(id) {
    fetch(`${API_URL}/promociones`, { credentials: 'include' })
        ...
        .then(data => {
            ...
            const precio = prompt('Precio destacado:', pr.precio_destacado || '');
...
async function loadHistorial() {
    ...
    try {
        const res = await fetchAuth('/reservas');
        const data = await res.json();
        const reservasDelDia = data.filter(r => new Date(r.fecha).toISOString().split('T')[0] === fecha);
        ...
        document.getElementById('historialResumen').innerHTML = `
            ...
            <div style="background: var(--dark2); border: 1px solid rgba(200,134,10,0.2); border-radius: 8px; padding: 1.2rem 2rem; text-align: center; flex: 1;"><div style="font-size: 2rem; color: var(--gold-l); font-weight: 600;">${pendientes}</div><div style="font-size: 0.8rem; color: var(--muted); text-transform: uppercase;">Pendientes</div></div>
        ...
        Object.keys(porHora).forEach(hora => {
            ...
            porHora[hora].forEach(r => {
                ...
                html += `<tr><td>${r.nombre || r.nombre_completo || '-'}</td><td>${r.sucursal || '-'}</td><td>${r.personas}</td><td>${r.telefono || '-'}</td><td>${r.notas || '-'}</td><td><span class="status-badge ${badge}">${r.estatus}</span></td></tr>`;
            });
